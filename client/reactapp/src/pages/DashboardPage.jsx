...
export default function DashboardPage() {
  ...
  const [watchlistLookupLoading, setWatchlistLookupLoading] = useState(false)
  const [watchlistLookupError, setWatchlistLookupError] = useState('')
  // keyed by ticker: { currentPrice, dayChange, dayChangePct, sector, fetchedAt, loading }
  ...
  async function handleWatchlistLookup() {
    if (!watchlistForm.ticker.trim()) return
    setWatchlistLookupLoading(true)
    setWatchlistLookupError('')
    try {
      const res = await fetch(`${API_BASE}/api/holdings/lookup/${encodeURIComponent(watchlistForm.ticker.trim())}`)
      const data = await res.json()
      if (!res.ok) {
        setWatchlistLookupError(data.error ?? 'Ticker not found.')
        return
      }
      setWatchlistForm((prev) => ({
        ...prev,
        name: data.name || prev.name,
        assetClass: data.assetClass || prev.assetClass || 'stock',
        // currentPrice intentionally NOT autofilled — user sets their own entry price
      }))
    } catch (err) {
      setWatchlistLookupError('Could not reach the server.')
    } finally {
      setWatchlistLookupLoading(false)
    }
  }
  ...
  async function fetchWatchlist() {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/watchlist/${USER_ID}`)
      const data = await res.json()
      setWatchlist(data)
      // Kick off live quote fetches for each unique ticker
      const uniqueTickers = [...new Set(data.map(i => i.ticker))]
      uniqueTickers.forEach(ticker => fetchWatchlistQuote(ticker))
    } catch (err) {
  ...
  async function fetchWatchlistQuote(ticker) {
    setWatchlistQuotes(prev => ({ ...prev, [ticker]: { ...prev[ticker], loading: true } }))
    try {
      const res = await fetch(`${API_BASE}/api/holdings/quote/${encodeURIComponent(ticker)}`)
      const data = await res.json()
      setWatchlistQuotes(prev => ({ ...prev, [ticker]: { ...data, loading: false } }))
    } catch {
      setWatchlistQuotes(prev => ({ ...prev, [ticker]: { loading: false } }))
    }
  }
  ...
  async function handleAddWatchlistItem() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // fetch live quote for newly added item
        fetchWatchlistQuote(data.ticker)
      } else {
  ...
  async function refreshWatchlistQuotes() {
    const uniqueTickers = [...new Set(watchlist.map((i) => i.ticker))]
    if (uniqueTickers.length === 0) return
    setWatchlistRefreshing(true)
    await Promise.all(uniqueTickers.map((ticker) => fetchWatchlistQuote(ticker)))
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full gap-0">
          ...
          <TabsContent value="watchlist" className="mt-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={refreshWatchlistQuotes} disabled={watchlistRefreshing || watchlist.length === 0}>
                  <RefreshCw className={cn('size-4 mr-1', watchlistRefreshing && 'animate-spin')} /> Refresh Prices
                </Button>
                <Button
                  size="sm"
                  onClick={() => {
                    setShowWatchlistForm(!showWatchlistForm)
                  }}
                >
                  <Plus className="size-4 mr-1" /> Add to Watchlist
                </Button>
              </div>
            </div>
              ...
              <Card className="p-4 mb-4">
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-wrap gap-3 items-end mb-3">
                    <div className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        onKeyDown={e => e.key === 'Enter' && handleWatchlistLookup()}
                        className="border rounded px-2 py-1 text-sm w-24 uppercase"
                      />
                    </div>
                    <Button size="sm" variant="outline" onClick={handleWatchlistLookup} disabled={watchlistLookupLoading || !watchlistForm.ticker.trim()}>
                      {watchlistLookupLoading ? 'Looking up...' : 'Look Up'}
                    </Button>
                    {watchlistLookupError && (
                      <p className="text-xs text-destructive self-end">{watchlistLookupError}</p>
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-3 items-end">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => setShowWatchlistForm(false)}>
                      Cancel
                    </Button>
                  </div>
                </div>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    ...
                    <SortHead>Sector</SortHead>
                    <SortHead className="text-right">Last Price</SortHead>
                    <SortHead className="text-right">Day Change</SortHead>
                    <SortHead className="text-right">Entry Price</SortHead>
                    <SortHead className="text-right">Added On</SortHead>
                    <TableHead />
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {watchlistLoading ? (
                    ...
                    <TableRow className="hover:bg-transparent">
                      <TableCell colSpan={9} className="py-10 text-center text-sm italic text-muted-foreground">
                        No watchlist items yet — click Add to Watchlist to get started.
                      </TableCell>
                    </TableRow>
                  ) : (
                    watchlist.map((item) => {
                      ...
                      const dayChange = q.dayChange ?? null
                      const dayChangePct = q.dayChangePct ?? null
                      const sector = q.sector ?? item.sector ?? null
                      const isUp = dayChange !== null ? dayChange >= 0 : null
                      const entryPrice = item.currentPrice ? Number(item.currentPrice) : null
                      const priceHitTarget = price !== null && entryPrice !== null && price <= entryPrice
                      const dropPct = priceHitTarget ? ((entryPrice - price) / entryPrice) * 100 : null
                      return (
                        <TableRow
                          key={item._id}
                        ...
                        >
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              {priceHitTarget && (
                                <TooltipProvider>
                                  <Tooltip>
                                    <TooltipTrigger asChild>
                                      <span className="inline-flex items-center rounded-full bg-green-500/15 px-1.5 py-0.5 text-[10px] font-medium text-green-500 ring-1 ring-inset ring-green-500/30">
                                        <TrendingUp className="mr-0.5 size-2.5" />
                                        At target
                                      </span>
                                    </TooltipTrigger>
                                    <TooltipContent side="right" className="text-xs">
                                      Live price is at or below your entry — potential buy opportunity
                                    </TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
                              )}
                          ...
                          <TableCell className="max-w-[130px] text-muted-foreground">
                            {item.name && item.name.length > 18 ? (
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger className="block max-w-full truncate text-left">{item.name}</TooltipTrigger>
                                  <TooltipContent side="top" className="text-xs">{item.name}</TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            ) : (item.name || '—')}
                          </TableCell>
                          ...
                          {/* Type */}
                          <TableCell className="w-16 capitalize text-muted-foreground text-xs">{item.assetClass ?? '—'}</TableCell>

                          {/* Sector */}
                          <TableCell className="max-w-[120px] text-muted-foreground">
                            {q.loading ? (
                              <span className="text-[10px] animate-pulse">…</span>
                            ) : sector && sector.length > 16 ? (
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger className="block max-w-full truncate text-left">{sector}</TooltipTrigger>
                                  <TooltipContent side="top" className="text-xs">{sector}</TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            ) : (sector || '—')}
                          </TableCell>
                          ...
                          <TableCell className="text-right">
                            {q.loading ? (
                              ...
                              <div>
                                ...
                                {q.fetchedAt && (
                                  <div className="text-[10px] text-muted-foreground/70">
                                    {new Date(q.fetchedAt).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                                  </div>
                                )}
                              </div>
                            )}
                          ...
                          <TableCell className="text-right">
                            {q.loading ? (
                              ...
                              <div>
                                ...
                                <div className={`text-[11px] ${isUp ? 'text-green-500' : 'text-red-500'}`}>
                                  {isUp ? '+' : ''}{Number(dayChangePct).toFixed(2)}%
                                </div>
                              </div>
                            ) : '—'}
                          </TableCell>

                          {/* Entry Price (price when added) */}
                          <TableCell className="text-right tabular-nums">
                            {entryPrice !== null ? (
                              priceHitTarget ? (
                                <div className="flex flex-col items-end gap-0.5">
                                  <span className="font-medium text-green-500">${entryPrice.toFixed(2)}</span>
                                  <span className="inline-flex items-center rounded-full bg-green-500/15 px-1.5 py-0.5 text-[10px] font-medium text-green-500 ring-1 ring-inset ring-green-500/30">
                                    -{dropPct.toFixed(2)}% below
                                  </span>
                                </div>
                              ) : (
                              ...
                              {item.notes && (
                                <TooltipProvider>
                                  <Tooltip>
                                    ...
                                    <TooltipContent side="left" className="max-w-[220px] text-xs">{item.notes}</TooltipContent>
                                  </Tooltip>
                                </TooltipProvider>
                              )}
                              <Button size="icon" variant="ghost" onClick={() => handleDeleteWatchlistItem(item._id)}>
                                <Trash2 className="size-4 text-red-500" />
                              </Button>
                            </div>
                          </TableCell>
